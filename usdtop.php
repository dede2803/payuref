...
function curl($url, $post = 0, $httpheader = 0, $proxy = 0)
{
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE, "u.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR, "u.txt");
...
if ($IP != "") {
    echo $WhiteTebal . "[23] SERVER USDT PAYU TOP";
    sleep(5);
    echo "\r                                                          \r";
    function head()
    {
    ...
    function get()
    {
        $url = "https://usdtpayu.top";
        return curl($url, '', head())[1];
    }

    function login($email, $token, $header)
    {
        $url = "https://usdtpayu.top/auth/login";
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://usdtpayu.top/faucet/currency/usdt");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    function currency()
    {
        $url = "https://usdtpayu.top/faucet/currency/usdt";
        return curl($url, '', head())[1];
    }

    function claim($aft, $ctn, $tkn, $header)
    {
        $url = "https://usdtpayu.top/faucet/verify/usdt";
    ...
    $login  = login($email, $token, array_merge(head(), array("Referer: https://usdtpayu.top/?r=15")));
    sleep(5);
    ...
    while (true) {
        for ($i = 0; $i < 500; $i++) {
            ...
            if ($sttsBal == "Empty") {
                echo $check2;
                echo $WhiteTebal . "[23] " . $YellowTebal . "USDT PAYU TOP " . $RedTebal . "Sedang Kosong. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php xrpTop.php");
            }

            $alert = explode(' text-center">', explode('<div class="alert alert-', $currency)[1])[0];
            if ($alert == "danger") {
                echo $WhiteTebal . "[23] Claim" . $YellowTebal . "USDT PAYU TOP " . $RedTebal . "Habis. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php xrpTop.php");
            } else {
                ...
                if ($aft == "") {
                    echo $WhiteTebal . "SERVER USDT PAYU TOP sedang " . $RedTebal . " Gangguan..!!!";
                    sleep(2);
                    echo "\r                                                                   \r";
                    sleep(2);
                    system("php xrpTop.php");
                    ...
                    wktClaim($timeClaim);
                    sleep(2);
                    $claim = claim($aft, $ctn, $tkn, array_merge(head(), array("Referer: https://usdtpayu.top/faucet/currency/usdt")));

                    echo $WhiteTebal . "Selamat, claim " . $GreenTebal . "BERHASIL.!!!";
                    sleep(2);
                    echo "\r                                                                \r";
                    sleep(2);
                    echo $WhiteTebal . "SERVER        = " . $YellowTebal . "USDT PAYU TOP\n";
                    sleep(2);
