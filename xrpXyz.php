...
function curl($url, $post = 0, $httpheader = 0, $proxy = 0)
{
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE, "w.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR, "w.txt");
...
if ($IP != "") {
    echo $WhiteTebal . "[21] SERVER XRP PAYU XYZ";
    sleep(5);
    echo "\r                                                          \r";
    function head()
    {
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://xrppayu.xyz/faucet/currency/xrp");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    function currency()
    {
        $url = "https://xrppayu.xyz/faucet/currency/xrp";
        return curl($url, '', head())[1];
    }

    function claim($aft, $ctn, $tkn, $header)
    {
        $url = "https://xrppayu.xyz/faucet/verify/xrp";
    ...
    $login  = login($email, $token, array_merge(head(), array("Referer: https://xrppayu.xyz/?r=3")));
    sleep(5);
    ...
    while (true) {
        for ($i = 0; $i < 500; $i++) {
            ...
            if ($sttsBal == "Empty") {
                echo $check2;
                echo $WhiteTebal . "[21] " . $YellowTebal . "XRP PAYU XYZ " . $RedTebal . "Sedang Kosong. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php zecTop.php");
            }

            $alert = explode(' text-center">', explode('<div class="alert alert-', $currency)[1])[0];
            if ($alert == "danger") {
                echo $WhiteTebal . "[21] Claim" . $YellowTebal . "XRP PAYU XYZ " . $RedTebal . "Habis. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php zecTop.php");
            } else {
                ...
                if ($aft == "") {
                    echo $WhiteTebal . "Server XRP PAYU XYZ sedang " . $RedTebal . " Gangguan..!!!";
                    sleep(2);
                    echo "\r                                                                   \r";
                    sleep(2);
                    system("php zecTop.php");
                    ...
                    wktClaim($timeClaim);
                    sleep(2);
                    $claim = claim($aft, $ctn, $tkn, array_merge(head(), array("Referer: https://xrppayu.xyz/faucet/currency/xrp")));

                    echo $WhiteTebal . "Selamat, claim " . $GreenTebal . "BERHASIL.!!!";
                    sleep(2);
                    echo "\r                                                                \r";
                    sleep(2);
                    echo $WhiteTebal . "SERVER        = " . $YellowTebal . "XRP PAYU XYZ\n";
                    sleep(2);
