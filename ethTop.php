...
function curl($url, $post = 0, $httpheader = 0, $proxy = 0)
{
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE, "k.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR, "k.txt");
...
if ($IP != "") {
    echo $WhiteTebal . "[11] SERVER ETH PAYU TOP";
    sleep(5);
    echo "\r                                                          \r";
    function head()
    {
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://ethpayu.top/faucet/currency/eth");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    function currency()
    {
        $url = "https://ethpayu.top/faucet/currency/eth";
        return curl($url, '', head())[1];
    }

    function claim($aft, $ctn, $tkn, $header)
    {
        $url = "https://ethpayu.top/faucet/verify/eth";
    ...
    $login  = login($email, $token, array_merge(head(), array("Referer: https://ethpayu.top/?r=7")));
    sleep(5);
    ...
    while (true) {
        for ($i = 0; $i < 500; $i++) {
            ...
            if ($sttsBal == "Empty") {
                echo $check2;
                echo $WhiteTebal . "[11] " . $YellowTebal . "ETH PAYU TOP " . $RedTebal . "Sedang Kosong. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php feyTop.php");
            }

            $alert = explode(' text-center">', explode('<div class="alert alert-', $currency)[1])[0];
            if ($alert == "danger") {
                echo $WhiteTebal . "[11] Claim" . $YellowTebal . "ETH PAYU TOP " . $RedTebal . "Habis. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php feyTop.php");
            } else {
                ...
                if ($aft == "") {
                    echo $WhiteTebal . "Server ETH PAYU TOP sedang " . $RedTebal . " Gangguan..!!!";
                    sleep(2);
                    echo "\r                                                                   \r";
                    sleep(2);
                    system("php feyTop.php");
                    ...
                    wktClaim($timeClaim);
                    sleep(2);
                    $claim = claim($aft, $ctn, $tkn, array_merge(head(), array("Referer: https://ethpayu.top/faucet/currency/eth")));

                    echo $WhiteTebal . "Selamat, claim " . $GreenTebal . "BERHASIL.!!!";
                    sleep(2);
                    echo "\r                                                                \r";
                    sleep(2);
                    echo $WhiteTebal . "SERVER        = " . $YellowTebal . "ETH PAYU TOP\n";
                    sleep(2);
