...
function curl($url, $post = 0, $httpheader = 0, $proxy = 0)
{
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE, "e.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR, "e.txt");
...
if ($IP != "") {
    echo $WhiteTebal . "[5] SERVER BNB PAYU XYZ";
    sleep(5);
    echo "\r                                                          \r";
    function head()
    {
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://bnbpayu.xyz/faucet/currency/bnb");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    function currency()
    {
        $url = "https://bnbpayu.xyz/faucet/currency/bnb";
        return curl($url, '', head())[1];
    }

    function claim($aft, $ctn, $tkn, $header)
    {
        $url = "https://bnbpayu.xyz/faucet/verify/bnb";
    ...
    $login  = login($email, $token, array_merge(head(), array("Referer: https://bnbpayu.xyz/?r=627")));
    sleep(5);
    ...
    while (true) {
        for ($i = 0; $i < 500; $i++) {
            ...
            if ($sttsBal == "Empty") {
                echo $check2;
                echo $WhiteTebal . "[5] " . $YellowTebal . "BNB PAYU XYZ " . $RedTebal . "Sedang Kosong. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php dashTop.php");
            }

            $alert = explode(' text-center">', explode('<div class="alert alert-', $currency)[1])[0];
            if ($alert == "danger") {
                echo $WhiteTebal . "[5] Claim" . $YellowTebal . "BNB PAYU XYZ " . $RedTebal . "Habis. " . $WhiteTebal . "Tunggu Besok Lagi !!!\n";
                sleep(2);
                system("php dashTop.php");
            } else {
                ...
                if ($aft == "") {
                    echo $WhiteTebal . "Server Bnb Payu Xyz sedang " . $RedTebal . " Gangguan..!!!";
                    sleep(2);
                    echo "\r                                                                   \r";
                    sleep(2);
                    system("php dashTop.php");
                    ...
                    wktClaim($timeClaim);
                    sleep(2);
                    $claim = claim($aft, $ctn, $tkn, array_merge(head(), array("Referer: https://bnbpayu.xyz/faucet/currency/bnb")));

                    echo $WhiteTebal . "Selamat, claim " . $GreenTebal . "BERHASIL.!!!";
                    sleep(2);
                    echo "\r                                                                \r";
                    sleep(2);
                    echo $WhiteTebal . "SERVER        = " . $YellowTebal . "BNB PAYU XYZ\n";
                    sleep(2);
